...
### 6.4 3단계: 강제 triage (`runs/stage3_b25`, 2026-07-25) — 그림의 완성

예산 2.5비트에서는 waterfill이 반드시 절반을 2비트/절반을 3비트로 나눠야 하므로
균일 배분이 존재하지 않는다. 이 조건에서는 **학습 중에도 정보 있는 순위가 이긴다**:

| 비교 (페어드, 3시드) | diff | 판정 |
|---|---|---|
| fisher − uniform(균형 반반) | **−0.0069 [전 시드 승리]** | 강제 triage에서 순위가 작동 |
| connectivity − uniform(균형 반반) | −0.0023 [전 시드 승리] | 〃 (fisher보다 약함) |
| connectivity − random | −0.0068 [전 시드 승리] | |

배포 다이얼에서의 신호 비교(uniform ckpt에 gradient 통계 사후 주입, 50 배치 캘리브레이션):
fisher 순위가 극단 압축(2.25b)에서 connectivity 대비 −0.023 [전 시드], 2.5b mixed,
2.75b 동률 — 학습 예산에 가까울수록 수렴한다.

### 6.5 종합 결론 (학습 45런 + 배포 스윕 2종)

1. **균일 배분이 가능한 조건(정수 예산)의 학습 중 재배분은 이득이 없다.** Fisher
   순위조차 uniform을 못 이긴다 — QAT 내생성(채널이 받은 정밀도에 적응)이 원인이며,
   이는 신호 결함이 아니라 구조적 현상이다. FracBits/QBitOpt류 학습 중 배분 연구가
   per-tensor에 머문 것과 정합적인 발견.
2. **배분의 실증 가치는 triage가 강제되는 곳에 있다**: (a) 분수 예산 학습,
   (b) **배포 시점 예산 축소** (재양자화 없는 다이얼 — 중첩 포맷의 고유 기능).
   후자에서 순위 효과가 가장 크다 (2.75b에서 균형 순위 대비 −0.023, random 대비 −0.104).
3. **신호 서열은 어디서나 일관된다**: fisher ≥ connectivity(≈flow) ≫ random.
   connectivity의 자리는 "gradient-free" — forward 캘리브레이션만으로 fisher의
   70~100% 효과. product의 norm 항은 무기여(ρ(product,flow)=0.98)이므로
   신호 정의는 사실상 **활성 흐름**이다.
4. **churn에는 실비용이 있고**(random_churn이 random보다 +0.009), 학습 예산과 다른
   배포 예산은 상향조차 손해다 — "학습=배포 일치" 원칙의 정량 근거.

**논문화 프레이밍**: "학습 중 동적 배분"(기각)이 아니라 —
"**중첩 비트 플레인으로 학습한 모델은 배포 예산 다이얼을 공짜로 얻고,
축소 시 희생 순위는 활성 흐름이 답한다. 학습 자체는 균일이 최적이라는 것을
Fisher 대조까지 통제해 보인 것**"이 정직하고 방어 가능한 기여다.
